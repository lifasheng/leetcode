...
public:
    int uniquePathsWithObstacles(vector<vector<int> > &obstacleGrid) {
        ...
        const int n = obstacleGrid[0].size();
        vector<vector<int> > cache(m+1, vector<int> (n+1, -1));
        return helper(obstacleGrid, cache, m, n);
    }
    
    int helper(vector<vector<int> > &obstacleGrid, vector<vector<int> > &cache, int m, int n) {
        if (m<1 || n<1) return 0;
        if (obstacleGrid[m-1][n-1]) return 0;
        if (m==1 && n==1) return 1;
        
        if (cache[m][n] == -1) {
            cache[m][n] = helper(obstacleGrid, cache, m-1, n) + helper(obstacleGrid, cache, m, n-1);
        }
        return cache[m][n];
    }
};
