...
// very good! both recursive and iterative!
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if (total % 2 != 0) {
            ...
            return (findKthSortedArrays(nums1, nums2, total / 2) + findKthSortedArrays(nums1, nums2, total / 2  + 1)) / 2.0;
        }
    }
    ...
    private int findKthSortedArrays(int[] nums1, int[] nums2, int k) {
        int m = nums1.length;
        int n = nums2.length;
        if (m > n) return findKthSortedArrays(nums2, nums1, k);
        ...
        if (nums1[a - 1] == nums2[b - 1]) return nums1[a - 1];
            ...
            return findKthSortedArrays(Arrays.copyOfRange(nums1, a, nums1.length), nums2, k - a);
        } else {
            return findKthSortedArrays(nums1, Arrays.copyOfRange(nums2, b, nums2.length), k - b);
        }
    } 
}
